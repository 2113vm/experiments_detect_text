...
from geometry import Contour, Rectangle, Point
import utils
import cv2
import numpy as np
...
def find_text_lines(original, thresh=0.02, min_size=4, max_size=12, spacing=1.8, min_length=1, otsu=False, x_only=False):
    def _threshold_normal(_image, _thresh):
        return np.uint8(cv2.threshold(_image, _thresh * utils.dtype_limits(_image)[1], 255, cv2.THRESH_BINARY)[1])
    ...
    # if x_only:
    #     diff = utils.differentiate(np.float32(_find_text_preprocess(original)) / 255., xkernel=5, ykernel=5, metric=utils.METRIC_SPLIT)[0]
    # else:
    #     diff = utils.differentiate(np.float32(_find_text_preprocess(original)) / 255., xkernel=5, ykernel=5)
    # diff = utils.differentiate(np.float32(_find_text_preprocess(original) / 255.), metric=utils.METRIC_MIN)
    ...
    connected = cv2.morphologyEx(binary, cv2.MORPH_CLOSE,
                                 cv2.getStructuringElement(cv2.MORPH_RECT, utils.fix_kernel_size(ksize, ksize)),
                                 borderType=cv2.BORDER_CONSTANT, borderValue=0)
    # cv2.imshow('connected', connected)
    # Filtering
    binary = cv2.morphologyEx(connected, cv2.MORPH_OPEN,
                              cv2.getStructuringElement(cv2.MORPH_RECT, utils.fix_kernel_size(int(min_size) - 1,
                                                                                              int(min_size) - 1)))
    # cv2.imshow('bin', binary)
    return filters.filter([Contour(c, thresh) for c in cv2.findContours(binary, cv2.RETR_EXTERNAL,
                                                                                   cv2.CHAIN_APPROX_SIMPLE)[0]])


def find_text(original=None, diffs=None, first_thresh=0.02, second_thresh=0.04, min_size=4, max_size=12,
              hspacing=1.8, vspacing=0, min_length=1, offset=(0, 0), otsu=False, split=True,
    ...
    sub = cv2.morphologyEx(binary, cv2.MORPH_OPEN,
                           cv2.getStructuringElement(cv2.MORPH_RECT, utils.fix_kernel_size(1, max_size)))
    ...
    # Connecting
    connected = cv2.morphologyEx(binary, cv2.MORPH_CLOSE,
                                 cv2.getStructuringElement(cv2.MORPH_RECT, utils.fix_kernel_size(ksize_h, ksize_v)),
                                 borderType=cv2.BORDER_CONSTANT, borderValue=0)
    if _debug:
        cv2.imshow("connected", connected)
    ...
    # Filtering
    binary = cv2.morphologyEx(connected, cv2.MORPH_OPEN,
                              cv2.getStructuringElement(cv2.MORPH_RECT, utils.fix_kernel_size(min_length * int(min_size) - 1,
                                                                                         int(min_size) - 1)))
    if _debug:
        cv2.imshow("connected and filtered", binary)

    # 1st detect_angle, not accurate, with max_thresh, not using offset yet
    conts = [Contour(c, first_thresh) for c in cv2.findContours(binary, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[0]]
    ...
    bin_for_ff = cv2.bitwise_or(second_bin[0], (connected / 255) * 2)
    ...
    conts = [Contour(c, first_thresh) for c in cv2.findContours(bin_for_ff, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE,
    ...
    if not split:
        ...
        for c in filtered:
            if c.rect.height >= 2 * min_size:
                splitted.extend(split_lines(bin_for_split, c, min_size, offset=(-offset[0], -offset[1])))
            else:
                splitted.append(c)
    ...
    if ret == TEXT_CONTOURS:
        ...
        boxes = [find_text_box(contour=c, bdiff=second_bin, max_rad=max_box_rad, default_rad=default_box_rad,
...
def find_text_box(contour, image=None, bdiff=None, max_rad=(32, 32), default_rad=(2, 2), raw=False, offset=(0, 0),
                  thresh=0.015):
    """
    :param bdiff: tuple of differences along each axis
    :param max_rad: tuple of max values (relative to the bounding rect of `contour` of box radius by each axis
    :param default_rad: tuple of default radius values that will be used if algorithm will be unable to detect_angle a box
    :return: Rectangle object
    """
    assert(image is not None or bdiff is not None)
    if bdiff is None:
        diffs = utils.differentiate(np.float32(cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)) / 255.,
                                    xkernel=5, ykernel=5, metric=utils.METRIC_SPLIT)
        bdiff = (np.uint8(cv2.threshold(diffs[0], thresh * utils.dtype_limits(diffs[0])[1], 255, cv2.THRESH_BINARY)[1]),
                 np.uint8(cv2.threshold(diffs[1], thresh * utils.dtype_limits(diffs[0])[1], 255, cv2.THRESH_BINARY)[1]))
    ...
    # Move left border
    left_arr = (bdiff[0][top, min_left:max_left] | bdiff[0][bottom, min_left:max_left]).nonzero()[0]
    ...
    # Move right border
    right_arr = (bdiff[0][top, min_right:max_right] | bdiff[0][bottom, min_right:max_right]).nonzero()[0]
    ...
    # Move bottom border
    bottom_arr = (bdiff[1][min_bottom:max_bottom, left] | bdiff[1][min_bottom:max_bottom, right]).nonzero()[0]
...
def split_lines(binary, contour, min_size, thresh=0.5, roi=False, offset=(0, 0)):
    ...
    for arg in args:  # TODO: make this code reusable for fragmentation
        ...
        conts = [Contour(c) for c in cv2.findContours(line_roi, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE,
                                                      offset=curr_off)[0]]
        lines.extend(conts)
        prev = arg
